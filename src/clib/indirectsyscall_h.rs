pub const INDIRECTSYSCALL_H: &str = r#"
#pragma once

#include <windows.h>

typedef enum FETCH_STATUS
{
	FETCH_STATUS_FAILED,
	FETCH_STATUS_SUCCESS,
	FETCH_STATUS_INVALID_PARAMETER,

} FETCH_STATUS; 

typedef enum NTDLL_INIT_STATUS 
{
	NTDLL_INIT_STATUS_FAILED,
	NTDLL_INIT_STATUS_SUCCESS,

} NTDLL_INIT_STATUS;

typedef enum NTSYSCALL_INIT_STATUS
{
	NTSYSCALL_INIT_STATUS_FAILED,
	NTSYSCALL_INIT_STATUS_SUCCESS,
	NTSYSCALL_INIT_STATUS_INVALID_PARAMETER,

} NTSYSCALL_INIT_STATUS;

typedef struct _NTDLL_CONFIG
{
	PDWORD pdwArrayOfAddresses;	// IMAGE_EXPORT_DIRECTORy.AddressOfFunctions
	PDWORD pdwArrayOfNames;		// IMAGE_EXPORT_DIRECTORy.AddressOfNames
	PWORD pwArrayOfOrdinals;	// IMAGE_EXPORT_DIRECTORy.AddressOfNameOrdinals
	DWORD dwNumberOfNames;		// IMAGE_EXPORT_DIRECTORy.NumberOfNames
	ULONG_PTR uModule;		// ntdll.dll base address
}NTDLL_CONFIG, *PNTDLL_CONFIG;

typedef struct _NT_SYSCALL
{
	DWORD dwSSn; 			// syscall nummber
	DWORD dwSyscallHash;		// syscall hash
	PVOID pSyscallAddress;		// syscall address
	PVOID pSyscallInstAddress;	// syscall address in ntdll
...
NTDLL_INIT_STATUS InitNtdllConfig(OUT PNTDLL_CONFIG pNtConfig);
...
#define HASH(API) HashStringJenkinsOneAtATime32BitA((char*)API)
